...
class EpicsAndStoriesMutator:
    """Workflow-aware mutations for EpicsAndStoriesDocument.

    Supports replace_section operation for any section.
    Full artifact rewrite allowed by bmad-create-epics-and-stories workflow.
    """

    def apply_operations(
        self, doc: EpicsAndStoriesDocument, workflow_id: str, operations: list[dict], contract
    ) -> EpicsAndStoriesDocument:
        ...
        return EpicsAndStoriesDocument(
            path=doc.path,
    ...
    def _rebuild_raw(self, project_name: str, status: str, sections: dict[str, str], contract_raw: dict) -> str:
        return _rebuild_markdown(
            f'Epics and Stories: {project_name}',
            {'Status': status},
            sections,
            contract_raw,
        )
