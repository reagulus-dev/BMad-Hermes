...
class ReviewRecordMutator:
    """
    Section-safe review record edits.
    Supports: checkbox, replace_section operations.
    """
    ...
    def apply_operations(
        self, doc: ReviewRecordDocument, workflow_id: str, operations: list[dict], contract
        ...
        for op in operations:
            op_type = op.get('type')
            if op_type == 'checkbox':
                # Checkboxes live in findings section
                if 'findings' not in sections:
                    raise ValueError('findings section not found in review record')
                sections['findings'] = self._checkbox_ops.set_checkbox(
                    sections['findings'], op['match_text'], bool(op.get('checked'))
                )
                ...
                if section_id not in allowed:
                    raise ValueError(f'Section {section_id} is not a valid mutation target for review_record')
                sections[section_id] = op.get('new_content', '')
            elif op_type == 'set_status':
                # Direct status field update
                new_status = op.get('status', '')
                new_raw = self._rebuild_raw(doc.story_key, doc.review_type, new_status, sections, raw)
        ...
        new_raw = self._rebuild_raw(doc.story_key, doc.review_type, doc.status, sections, raw)
        return ReviewRecordDocument(
            path=doc.path,
            story_key=doc.story_key,
            review_type=doc.review_type,
    ...
    def _mutation_target_sections(self) -> set[str]:
        return {'review_summary', 'findings', 'recommendations', 'follow_up', 'review_agent_record'}

    def _rebuild_raw(
        self, story_key: str, review_type: str, status: str, sections: dict[str, str], contract_raw: dict
    ) -> str:
